...
namespace ATFramework.WebElements.Utils
{
	...
	{
		private static IEnumerable<XElement> _classSection;
		...
		{		  
			...
			{
				_classSection = from sct in document.Descendants("class")
			...
			if (_classSection == null || _classSection.Count() == 0)
			{
			...
			string locator = _classSection.Descendants("field")
				.ToList()
